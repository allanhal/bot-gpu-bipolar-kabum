...
const coupons = ["EMERSONBR", "ALANZOKA"];
...
class Kabum {
  ...
  async login() {
    ...
    this.page.click(
      'input[src="https://static.kabum.com.br/conteudo/temas/001/imagens/login/botao_logar.png"]'
    );
    await this.page.waitForNavigation();
  }
  ...
  async cheapAvailable(text_search) {
    await this.page.waitForSelector("#listagem-produtos");

    const itemNome = await this.page.$eval(
      ".item-nome",
      (element) => element.innerText
    );
    const disponivel = await this.page.$eval(
      `img[alt="botao comprar"]`,
      (element) => {
        console.log("element", element);
        return element.src;
      }
    );
    if (itemNome.includes(text_search) && disponivel.includes("comprar")) {
  ...
  async check_price(text_search, selected_word) {
    ...
    try {
      const priceText = await this.page.$eval(
        ".preco_desconto_avista-cm, .preco_desconto > span > span > strong",
        (element) => element.innerText
      );
      ...
      if (selected_word === tracked_words[1]) {
        if (priceFloat > preco_limite_2070) {
          console.log(
            "[ERROR] O PREÇO DO PRODUTO ESTÁ ACIMA DO LIMITE - " +
              text_search +
              " (" +
              priceFloat +
              ")"
      ...
      console.log(
        "[INFO] PREÇO DO PRODUTO VÁLIDO PARA O LIMITE DA CATEGORIA (" +
          priceFloat +
          ") - " +
          text_search
      );
  ...
  async add_to_cart(text_search) {
    try {
      ...
      console.log(
        "[ESGOTADA] " + new Date().toLocaleString() + " - " + text_search
      );
      this.browser.close();
      throw err;
    }
  ...
  async next_cart_page() {
    try {
      this.page.click(
        'a[href="https://www.kabum.com.br/cgi-local/site/carrinho/carrinho.cgi"]'
      );
      await this.page.waitForNavigation();
    } catch (err) {
      console.log(
        "[ERROR] OCORREU UM ERRO AO CLICAR EM PROCEDER NO PARA CHECKOUT.",
        err
      );
      this.browser.close();
      throw err;
  ...
  async checkout() {
    const [buttonFinalizarCompraPrev] = await this.page.$x(
    ...
    if (buttonFinalizarCompraPrev) {
      ...
      console.log(
        "[ERROR] Ocorreu um erro ao clicar no botão de Finalizar Compra 1"
      );
      this.browser.close();
      throw err;
    }
